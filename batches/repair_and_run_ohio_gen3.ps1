...
$Gen3Path = ".\batches\auto_generate_city_patterns.ps1"
New-Item -ItemType Directory -Force .\batches | Out-Null
...
$Lines | Set-Content -Encoding UTF8 $Gen3Path

$null = [scriptblock]::Create((Get-Content $Gen3Path -Raw))
Write-Host "Gen3 parser check: PASS"

$OhioAudit = Get-ChildItem ".\logs\black_dragon\full_saturation\wave_*_full_saturation_audit.json" |
  Where-Object { (Get-Content $_.FullName -Raw | ConvertFrom-Json).cities -contains "Columbus" } |
  Sort-Object LastWriteTime |
  Select-Object -Last 1

if (!$OhioAudit) { throw "Ohio wave audit not found." }

$OhioChecklistCsv = (Get-Content $OhioAudit.FullName -Raw | ConvertFrom-Json).checklist_csv
Write-Host "Ohio checklist:" $OhioChecklistCsv

$Rows = Import-Csv $OhioChecklistCsv
foreach ($Row in $Rows) {
  if ($Row.saturation_status -ne "verified_layer") {
    $Row.source_name = ""
    $Row.source_url = ""
    $Row.source_type = ""
    $Row.city_specific = "pending"
    $Row.provenance_status = "pending"
    $Row.authentication_status = "pending"
    $Row.replay_status = "pending"
    $Row.saturation_status = "gap_unfilled"
    $Row.eligible_for_lock = "false"
    $Row.notes = "Reset for Gen3 Ohio repair."
  }
}
$Rows | Export-Csv -NoTypeInformation -Encoding UTF8 $OhioChecklistCsv

pwsh -File .\batches\auto_generate_city_patterns.ps1 -ChecklistCsv $OhioChecklistCsv
pwsh -File .\batches\confirm_verified_parity_layers.ps1 -ChecklistCsv $OhioChecklistCsv
pwsh -File .\batches\strict_validate_full_saturation.ps1 -ChecklistCsv $OhioChecklistCsv
pwsh -File .\batches\audit_full_saturation.ps1 -ChecklistCsv $OhioChecklistCsv
