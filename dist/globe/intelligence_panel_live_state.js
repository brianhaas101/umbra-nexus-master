(function () {
  ...
  function getRegion() {
    return safe(() =>
      ...
      window.UmbraGlobe?.state?.activeRegion || window.UmbraGlobe?.state?.activeCityMeta?.name ||
      "GLOBAL",
      "GLOBAL"
    );
  }
  ...
  function setField(root, label, value) {
    ...
    if (labelNode && labelNode.parentElement) {
      ...
      if (valueNode) {
        valueNode.textContent = String(value ?? "â€”");
        return true;
      }
    }
  ...
  function updateFallback(root, snap) {
    ...
    for (const [key, value] of Object.entries(rows)) {
      ...
      if (row) row.textContent = String(value ?? "â€”");
    }
  }
