  ...
  G.buildLayers = function buildLayers(tex = {}) {
    try {
      ...
      const globeGeo = new THREE.SphereGeometry(radius, 144, 144);
        ...
        map: (baseTex && baseTex.image && baseTex.image.width)
          ? baseTex
          : window.UmbraGlobe.makeDeterministicEarthFallbackTexture(),
        normalMap: normalMap,
      ...
      if (baseTex && baseTex.image && baseTex.image.width) {
        globeMat.map = baseTex;
        globeMat.needsUpdate = true;
        console.log("[layers] base texture rebound after globe create", globeMat.map?.userData);
      }
        normalMap: normalMap,
        normalScale: new THREE.Vector2(0.58, 0.58),
        metalnessMap: specularMap,
