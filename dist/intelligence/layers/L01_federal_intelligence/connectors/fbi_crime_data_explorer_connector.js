...
export const SOURCE_ID = "fbi_crime_data_explorer";
...
export async function fetchRawRecords() {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      "User-Agent": "UmbraNexus-L01-FBI-CDE/1.0"
    }
  });

  if (!response.ok) throw new Error(`FBI CDE failed: ${response.status}`);
  ...
  return [
    makeRawEnvelope(
      ...
      Object.freeze({
        endpoint: ENDPOINT,
        response_type: "fbi_cde_agencies",
        payload
      }),
      "fbi_cde_agencies"
    )
