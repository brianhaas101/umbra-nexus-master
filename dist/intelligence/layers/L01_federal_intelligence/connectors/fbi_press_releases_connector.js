...
const ENDPOINT = "https://www.fbi.gov/news";
...
export async function fetchRawRecords() {
  const response = await fetch(ENDPOINT, {
    headers: {
      "Accept": "text/html",
      "User-Agent": "UmbraNexus-L01-FBI/1.0"
    }
  });

  if (!response.ok) {
    throw new Error(`FBI request failed: ${response.status}`);
  }
  ...
  return [
    makeRawEnvelope(
      Object.freeze({
        source_id: SOURCE_ID,
        source_name: "FBI News",
        canonical_url: ENDPOINT,
        access_method: "public_html"
      }),
      Object.freeze({
        endpoint: ENDPOINT,
        response_type: "fbi_news_html",
        html
      }),
      "fbi_news_source_page"
    )
  ];
}
