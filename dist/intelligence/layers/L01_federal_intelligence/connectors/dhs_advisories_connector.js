...
const ENDPOINT = "https://www.dhs.gov/news";
...
export async function fetchRawRecords() {
  ...
  if (!response.ok) {
    throw new Error(`DHS news request failed: ${response.status}`);
  }
  ...
  return [
    makeRawEnvelope(
      ...
      Object.freeze({
        endpoint: ENDPOINT,
        response_type: "dhs_news_html",
        html
      }),
      "dhs_advisories_source_page"
    )
  ];
}
