export const SOURCE_ID = "state_business_regulation";
const ENDPOINT = "https://www.dca.ca.gov/";

export async function fetchRawRecords() {
  const response = await fetch(ENDPOINT, {
    headers: { "Accept": "text/html", "User-Agent": "UmbraNexus-L02-CA-DCA/1.0" }
  });
  if (!response.ok) throw new Error(`CA DCA failed: ${response.status}`);
