...
(function () {
  function boot() {
    ...
    const makeClientCanvasTexture = () => {
      ...
      g.addColorStop(0.45, "#102033");
      g.addColorStop(1, "#05070a");
      ctx.fillStyle = g;
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      ctx.fillStyle = "rgba(255, 115, 24, 0.35)";
      for (let i = 0; i < 900; i++) {
      ...
      tex.userData = { __umbraOwned: true, __umbraUrl: "generated-client-canvas" };
      tex.needsUpdate = true;
      return tex;
    };

    const remoteClientTex = makeClientCanvasTexture();
    ...
    G.loadTextures(
      ...
      (tex) => {
        if (!isLocalRuntime) {
          tex = {
            base: remoteClientTex,
            night: remoteClientTex,
            starfield: null
          };

          if (!window.UMBRA_DATA?.datasetHash) {
            console.warn("[scene] remote client waiting for UMBRA_DATA before build");
        ...
        try {
          ...
          function afterDataReady() {
            ...
            if (!payload?._src?.datasetHash) {
              console.warn("[scene] datasetHash missing after load; client mode continuing.");
            }
