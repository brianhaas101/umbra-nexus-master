export const SOURCE_ID = "state_energy_utilities";
const ENDPOINT = "https://www.energy.ca.gov/";

export async function fetchRawRecords() {
  const response = await fetch(ENDPOINT, {
    headers: { "Accept": "text/html", "User-Agent": "UmbraNexus-L02-CA-Energy/1.0" }
  });
  if (!response.ok) throw new Error(`CA Energy failed: ${response.status}`);
