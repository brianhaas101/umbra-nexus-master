...
const report = {
  version: "nexus_L01_live_depth_audit_v2",
...
for (const connector of registry.connectors) {
  ...
  const isLive = state.status === "LIVE_FETCH_SUCCESS";
  const isReference = state.status && state.status.startsWith("REFERENCE_READY");
  const isManual = state.status === "MANUAL_REVIEW_REQUIRED";
  const isBlocked = state.status === "LIVE_FETCH_EMPTY_OR_FAILED" && Boolean(state.binding_required);

  if (isLive) report.counts.live_fetch_success++;
  else if (isReference) report.counts.reference_ready++;
  else if (isManual) report.counts.manual_review_required++;
  else if (isBlocked) report.counts.blocked_requires_binding++;
  else {
    report.counts.invalid_failed++;
  ...
  if (isLive || isReference || isManual) {
  ...
  if (isBlocked) {
    if (evidence !== 0) fail(`${source_id}_BLOCKED_EVIDENCE_NOT_ZERO`);
    if (signals !== 0) fail(`${source_id}_BLOCKED_SIGNALS_NOT_ZERO`);
    if (scores !== 0) fail(`${source_id}_BLOCKED_SCORES_NOT_ZERO`);
  }
...
writeJson("logs/sophistication/L01_live_depth_audit_v2.json", report);
