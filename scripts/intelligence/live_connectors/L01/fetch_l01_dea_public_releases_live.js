...
function extractLinks(html) {
  ...
  while ((match = re.exec(html)) !== null) {
    ...
    if (/press|release|operation|trafficking|drug|arrest|seizure|enforcement/i.test(label + " " + href)) {
...
async function main() {
  ...
  const source_id = "L01_SRC_007_DEA_PUBLIC_RELEASES";
  ...
  const rawSnapshot = {
    ...
    request: {
      method: "GET",
      purpose: "dea_press_release_live_fetch"
    },
  ...
  const extractedLinks =
    rawSnapshot.response?.extracted_links || [];

  const liveSuccess =
    rawSnapshot.status === "FETCH_SUCCESS" &&
    extractedLinks.length > 0;

  const evidence = liveSuccess
    ? extractedLinks.map((item, index) => ({

        evidence_id:
          `LIVE_DEA_RELEASE_EVIDENCE_${String(index + 1).padStart(3, "0")}`,
        ...
        lineage: {
          ...
          parser_id: "DEA_PRESS_RELEASE_LINK_PARSER_V1",
  ...
  const normalizedSnapshot = {
    ...
    signals: evidence.map((e, index) => ({
      ...
      signal_id:
        `LIVE_DEA_RELEASE_SIGNAL_${String(index + 1).padStart(3, "0")}`,

      layer_id: "L01",
      source_id,
      connector_id: state.connector_id,

      signal_type: "DEA_PUBLIC_RELEASE_LIVE_SIGNAL",
    ...
    score_components: evidence.map((e, index) => ({
      ...
      component_id:
        `LIVE_DEA_RELEASE_SCORE_${String(index + 1).padStart(3, "0")}`,
  ...
  normalizedSnapshot.normalized_hash =
    sha(normalizedSnapshot);
  ...
  const nextState = {
    ...
    last_success_at: liveSuccess
      ? now
      : state.last_success_at,

    last_failure_at: liveSuccess
      ? null
      : now,

    failure_count: liveSuccess
      ? 0
      : (state.failure_count || 0) + 1,

    last_error: liveSuccess
      ? null
      : {
    ...
    binding_required: liveSuccess
      ? null
      : "DEA_PUBLIC_RELEASE_QUERY_OR_PARSER_REVIEW",
  ...
  console.log(JSON.stringify({
    ...
    status:
      "L01_DEA_PUBLIC_RELEASES_LIVE_FETCH_COMPLETE",

    fetch_status: rawSnapshot.status,

    http_status:
      rawSnapshot.response?.status || null,
    ...
    score_components:
      normalizedSnapshot.score_components.length,
