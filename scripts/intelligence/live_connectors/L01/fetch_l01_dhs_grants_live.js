...
function extractLinks(html) {
  ...
  while ((match = re.exec(html)) !== null) {
    ...
    if (/grant|preparedness|homeland|security|emergency|funding|assistance|program/i.test(label + " " + href)) {
...
async function main() {
  ...
  const endpoint = "https://www.dhs.gov/grants";
  ...
  const rawSnapshot = {
    ...
    request: {
      method: "GET",
      purpose: "dhs_homeland_security_grants_live_page_fetch"
    },
    response: null,
    status: "PENDING"
  };
  ...
  const evidence = extractedLinks.map((item, index) => ({
    evidence_id: `LIVE_DHS_GRANTS_EVIDENCE_${String(index + 1).padStart(3, "0")}`,
    layer_id: "L01",
    source_id,
    connector_id: state.connector_id,
    source_name: state.source_name,
    ...
    lineage: {
      ...
      parser_id: "DHS_GRANTS_LINK_PARSER_V1",
      normalizer_id: "L01_FEDERAL_NORMALIZER_V1"
    },
    record: {
      label: item.label,
      href: item.href,
      record_type: "dhs_grant_program_or_preparedness_reference"
    }
  }));
  ...
  const normalizedSnapshot = {
    ...
    signals: evidence.map((e, index) => ({
      signal_id: `LIVE_DHS_GRANTS_SIGNAL_${String(index + 1).padStart(3, "0")}`,
      layer_id: "L01",
      source_id,
      connector_id: state.connector_id,
      signal_type: "FEDERAL_DHS_GRANT_PROGRAM_LIVE_SIGNAL",
      strength: 0.92,
      confidence: e.confidence
    })),
    score_components: evidence.map((e, index) => ({
      component_id: `LIVE_DHS_GRANTS_SCORE_${String(index + 1).padStart(3, "0")}`,
      layer_id: "L01",
      source_id,
      connector_id: state.connector_id,
      score_delta: 9,
  ...
  const nextState = {
    ...
    binding_required: liveSuccess ? null : "DHS_GRANT_PAGE_QUERY_OR_PARSER_REVIEW",
  ...
  console.log(JSON.stringify({
    status: "L01_DHS_GRANT_PROGRAMS_LIVE_FETCH_COMPLETE",
