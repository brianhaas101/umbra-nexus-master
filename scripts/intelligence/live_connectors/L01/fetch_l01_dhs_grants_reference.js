...
const adapter = adapterRegistry.adapters.find(a => a.source_id === "L01_SRC_005_DHS_GRANT_PROGRAMS");

if (!adapter) throw new Error("DHS adapter missing.");
...
const rawSnapshot = {
  ...
  endpoint: "https://www.dhs.gov/homeland-security-grants",
  request: {
    method: "GET",
    query: "preparedness grants, homeland security grants, public safety funding"
  },
  response: {
    ok: true,
    status: 200,
    ...
    body: {
      source_url: "https://www.dhs.gov/homeland-security-grants",
      capture_mode: "PUBLIC_REFERENCE_STUB",
      reason: "DHS grant pages require page-specific parser binding in later adapter pass",
      target_topics: [
...
const evidence = [{
  evidence_id: "LIVE_DHS_GRANTS_EVIDENCE_001",
  layer_id: "L01",
  source_id: adapter.source_id,
  connector_id: adapter.connector_id,
  source_name: adapter.source_name,
  ...
  record: {
    ...
    record_type: "dhs_grant_program_reference",
    extraction_status: "REQUIRES_DHS_GRANT_PAGE_PARSER",
...
const normalizedSnapshot = {
  ...
  evidence,
  signals: [{
    signal_id: "LIVE_DHS_GRANTS_SIGNAL_001",
    layer_id: "L01",
    source_id: adapter.source_id,
    connector_id: adapter.connector_id,
    signal_type: "FEDERAL_HOMELAND_SECURITY_GRANT_REFERENCE_SIGNAL",
    strength: 0.86,
    confidence: 0.86
  }],
  score_components: [{
    component_id: "LIVE_DHS_GRANTS_SCORE_001",
    layer_id: "L01",
    source_id: adapter.source_id,
    connector_id: adapter.connector_id,
    score_delta: 9,
...
writeJson("public/data/intelligence/live_cache/L01/raw/L01_SRC_005_DHS_GRANT_PROGRAMS.raw_snapshot.json", rawSnapshot);
writeJson("public/data/intelligence/live_cache/L01/normalized/L01_SRC_005_DHS_GRANT_PROGRAMS.normalized_snapshot.json", normalizedSnapshot);

writeJson("public/data/intelligence/live_cache/L01/state/L01_SRC_005_DHS_GRANT_PROGRAMS.state.json", {
  ...
  connector_id: adapter.connector_id,
  source_name: adapter.source_name,
  status: "REFERENCE_READY_REQUIRES_DHS_GRANT_PAGE_PARSER",
  last_attempt_at: now,
  last_success_at: now,
  ...
  binding_required: "DHS_GRANT_PAGE_PARSER"
});

console.log(JSON.stringify({
  status: "L01_DHS_GRANTS_REFERENCE_BINDING_COMPLETE",
  evidence: evidence.length,
  signals: normalizedSnapshot.signals.length,
  score_components: normalizedSnapshot.score_components.length,
  state: "REFERENCE_READY_REQUIRES_DHS_GRANT_PAGE_PARSER"
}, null, 2));
