...
if (selected.length !== 5) {
  throw new Error(`Expected 5 L01 batch 2 connectors, found ${selected.length}`);
}

const now = new Date().toISOString();

const execution_manifest = {
  version: "nexus_L01_live_ingestion_batch_002_manifest_v1",
  generated_at: now,
  layer_id: "L01",
  batch_id: "L01_LIVE_BATCH_002",
  batch_name: "Federal funding and procurement spine",
  connector_count: selected.length,
  execution_mode: "deterministic_live_ready",
  connectors: selected.map((connector, index) => ({
    execution_id: `L01_BATCH_002_EXEC_${String(index + 1).padStart(3, "0")}`,
...
console.log(JSON.stringify({
  status: "L01_LIVE_BATCH_002_MANIFEST_BUILT",
  batch_id: "L01_LIVE_BATCH_002",
  connectors: selected.length,
  sources: selected.map(c => c.source_id)
}, null, 2));
