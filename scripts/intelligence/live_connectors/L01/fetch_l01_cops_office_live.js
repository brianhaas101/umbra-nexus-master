...
function extractLinks(html) {
  ...
  while ((match = re.exec(html)) !== null) {
    ...
    if (
      /grant|program|funding|award|solicitation|opportunity/i.test(label) ||
      /grant|program|funding|award|solicitation|opportunity/i.test(href)
    ) {
...
async function main() {
  ...
  const rawSnapshot = {
    ...
    request: {
      method: "GET",
      purpose: "cops_office_grants_program_live_page_fetch"
    },
    response: null,
    status: "PENDING"
  };
  ...
  const evidence = extractedLinks.map((item, index) => ({
    evidence_id: `LIVE_COPS_PROGRAM_EVIDENCE_${String(index + 1).padStart(3, "0")}`,
    layer_id: "L01",
    source_id,
    connector_id: state.connector_id,
    source_name: state.source_name,
    ...
    lineage: {
      ...
      parser_id: "COPS_GRANT_PAGE_LINK_PARSER_V1",
      normalizer_id: "L01_FEDERAL_NORMALIZER_V1"
    },
    record: {
      label: item.label,
      href: item.href,
      record_type: "cops_office_program_or_grant_reference"
    }
  }));
  ...
  const normalizedSnapshot = {
    ...
    signals: evidence.map((e, index) => ({
      signal_id: `LIVE_COPS_PROGRAM_SIGNAL_${String(index + 1).padStart(3, "0")}`,
      layer_id: "L01",
      source_id,
      connector_id: state.connector_id,
      signal_type: "FEDERAL_COPS_PROGRAM_LIVE_SIGNAL",
      strength: 0.92,
      confidence: e.confidence
    })),
    score_components: evidence.map((e, index) => ({
      component_id: `LIVE_COPS_PROGRAM_SCORE_${String(index + 1).padStart(3, "0")}`,
      layer_id: "L01",
      source_id,
      connector_id: state.connector_id,
      score_delta: 9,
  ...
  console.log(JSON.stringify({
    status: "L01_COPS_OFFICE_PROGRAMS_LIVE_FETCH_COMPLETE",
