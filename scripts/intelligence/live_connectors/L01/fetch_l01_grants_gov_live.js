...
async function main() {
  const source_id = "L01_SRC_003_GRANTS_GOV";
  ...
  const payload = {
    keyword: "public safety law enforcement training",
  ...
  const rawSnapshot = {
    ...
    request: {
      method: "POST",
      payload,
      purpose: "grants_gov_public_safety_opportunity_search"
    },
    response: null,
    status: "PENDING"
  };
  ...
  const evidence = rows.map((item, index) => ({
    evidence_id: `LIVE_GRANTS_GOV_EVIDENCE_${String(index + 1).padStart(3, "0")}`,
    layer_id: "L01",
    source_id,
    connector_id: state.connector_id,
    source_name: state.source_name,
    ...
    lineage: {
      ...
      parser_id: "GRANTS_GOV_SEARCH2_PARSER_V1",
  ...
  const normalizedSnapshot = {
    ...
    signals: evidence.map((e, index) => ({
      signal_id: `LIVE_GRANTS_GOV_SIGNAL_${String(index + 1).padStart(3, "0")}`,
      layer_id: "L01",
      source_id,
      connector_id: state.connector_id,
      signal_type: "FEDERAL_GRANT_OPPORTUNITY_LIVE_SIGNAL",
      strength: 0.95,
      confidence: e.confidence
    })),
    score_components: evidence.map((e, index) => ({
      component_id: `LIVE_GRANTS_GOV_SCORE_${String(index + 1).padStart(3, "0")}`,
      layer_id: "L01",
      source_id,
      connector_id: state.connector_id,
      score_delta: 10,
  ...
  const nextState = {
    ...
    binding_required: liveSuccess ? null : "GRANTS_GOV_SEARCH2_PARSER_OR_QUERY_REVIEW",
  ...
  console.log(JSON.stringify({
    status: "L01_GRANTS_GOV_LIVE_FETCH_COMPLETE",
