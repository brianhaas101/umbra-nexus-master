...
const queue = gap.gaps
  .map(g => ({
    layer_id: g.layer_id,
    priority: g.severity === "HIGH" ? 1 : g.severity === "MEDIUM" ? 2 : 3,
    build_type: g.gap_type,
    status: g.current_coverage === "PRODUCTION_STRUCTURED" ? "PRODUCTION_STRUCTURED" : "QUEUED",
    required_next_step:
      g.current_coverage === "PRODUCTION_STRUCTURED"
        ? "IMPLEMENT_REAL_ADAPTERS_AND_CONNECTORS"
        : "BUILD_FULL_SOURCE_CATALOG_AND_ADAPTER"
  }))
  .sort((a, b) => a.priority - b.priority || a.layer_id.localeCompare(b.layer_id));

const output = {
  version: "nexus_intelligence_priority_build_queue_v2",
...
console.log("[PRIORITY QUEUE V2] COMPLETE", output.total_queue);
