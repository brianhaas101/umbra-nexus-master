...
const runtime = read("public/data/clients/black_dragon/city_runtime/scottsdale/merged/scottsdale_merged_city_entities.json");
const graph = read("public/data/clients/black_dragon/relationship_graph/scottsdale/edges/scottsdale_graph_edges.json");
const scores = read("public/data/clients/black_dragon/relationship_graph/scottsdale/scores/scottsdale_graph_influence_scores.json");
const paths = read("public/data/clients/black_dragon/relationship_graph/scottsdale/paths/scottsdale_propagation_paths.json");
const validation = read("public/data/clients/black_dragon/automation/live_validation/scottsdale/results/live_http_validation_results.json");
const contactReview = read("public/data/clients/black_dragon/automation/live_validation/scottsdale/contact_review/contact_route_review.json");
const deadRoutes = read("public/data/clients/black_dragon/automation/live_validation/scottsdale/dead_routes/dead_route_review_queue.json");

const panel = {
  version: "black_dragon_scottsdale_client_overview_panel_v1",
  generated_at: new Date().toISOString(),
  city: "Scottsdale",
  state: "AZ",
  panel_id: "BD_SCT_CLIENT_OVERVIEW",
  panel_title: "Scottsdale Operational Intelligence",
  client_visible: true,
  summary_cards: [
    { card_id: "SCT_RUNTIME_ENTITIES", label: "Runtime Entities", count: runtime.deduped_city_entities },
    { card_id: "SCT_CONTACT_CANDIDATES", label: "Manual Contact Candidates", count: contactReview.contact_ready_candidates },
    { card_id: "SCT_GRAPH_EDGES", label: "Graph Edges", count: graph.total_edges },
    { card_id: "SCT_PROPAGATION_PATHS", label: "Propagation Paths", count: paths.total_paths },
    { card_id: "SCT_ROUTE_REVIEW", label: "Route Reviews", count: deadRoutes.review_item_count },
    { card_id: "SCT_REGIONAL_DUPLICATES", label: "Regional Overlaps", count: runtime.duplicate_review_entities }
  ],
  hardlocks: {
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/scottsdale/ui/panels/scottsdale_client_overview_panel.json"),
...
const nodeCards = runtime.merged_entities.map(entity => {
  ...
  return {
    node_card_id: `BD_SCT_NODE_CARD_${String(entity.city_rank).padStart(5, "0")}`,
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/scottsdale/ui/node_cards/scottsdale_client_node_cards.json"),
  JSON.stringify({ version: "black_dragon_scottsdale_client_node_cards_v1", generated_at: new Date().toISOString(), total_node_cards: nodeCards.length, node_cards: nodeCards }, null, 2),
  "utf8"
);

const feed = {
  version: "black_dragon_scottsdale_propagation_feed_v1",
  generated_at: new Date().toISOString(),
  city: "Scottsdale",
  state: "AZ",
  client_visible: true,
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/scottsdale/ui/propagation_feed/scottsdale_client_propagation_feed.json"),
  JSON.stringify(feed, null, 2),
  "utf8"
);

const uiAudit = {
  version: "black_dragon_batch_167_scottsdale_ui_surface_audit_v1",
  generated_at: new Date().toISOString(),
  batch: "167_SCOTTSDALE_UI_AND_PRODUCTION_AUDIT",
  counts: {
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/scottsdale/ui/audit/batch_167_scottsdale_ui_surface_audit.json"),
  JSON.stringify(uiAudit, null, 2),
  "utf8"
);

const prodAudit = {
  version: "black_dragon_scottsdale_production_readiness_audit_v1",
  generated_at: new Date().toISOString(),
  city: "Scottsdale",
  state: "AZ",
  certification_target: "ARIZONA_SECOND_OPERATIONAL_CITY",
  counts: {
  ...
  certification: "SCOTTSDALE_OPERATIONAL_CITY_APPROVED",
  next_phase: "BATCH_168_ARIZONA_TWO_CITY_COMPARISON_AND_MESA_SELECTION",
  status: "PASS"
};

const out = path.join(ROOT, "public/data/clients/black_dragon/production_readiness/scottsdale/audit/scottsdale_production_readiness_audit.json");
fs.mkdirSync(path.dirname(out), { recursive: true });
fs.writeFileSync(out, JSON.stringify(prodAudit, null, 2), "utf8");

console.log(JSON.stringify({
  status: "BATCH_167_SCOTTSDALE_PRODUCTION_READINESS_AUDIT_COMPLETE",
