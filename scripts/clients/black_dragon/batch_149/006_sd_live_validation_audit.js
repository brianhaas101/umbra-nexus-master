...
const audit = {
  version:
    "black_dragon_batch_149_san_diego_live_validation_audit_v1",

  generated_at:
    new Date().toISOString(),

  batch:
    "149_SAN_DIEGO_LIVE_VALIDATION",
  ...
  counts: {
    ...
    valid_routes:
      results.validation_results.filter(r => r.fetch_status === "VALID").length,

    review_routes:
      results.validation_results.filter(r => r.fetch_status !== "VALID").length,

    freshness_records:
      freshness.freshness_records.length,

    dead_route_review_items:
      deadRoutes.review_item_count,

    contact_review_records:
      contactReview.review_count,
  ...
  gates: {
    validation_targets_exist:
      targets.validation_targets.length === 10,

    validation_results_match_targets:
      results.validation_count === 10,

    freshness_records_match_results:
      freshness.freshness_records.length === 10,

    contact_review_matches_runtime:
    ...
    no_runtime_mutation:
      contactReview.contact_review.every(
        r => r.runtime_mutation_allowed === false
      ),

    no_automatic_delete:
      deadRoutes.dead_route_items.every(
        r => r.automatic_delete_allowed === false
      )
  },
...
const out = path.join(
  ROOT,
  "public/data/clients/black_dragon/automation/live_validation/san_diego/audit/batch_149_san_diego_live_validation_audit.json"
);

fs.writeFileSync(out, JSON.stringify(audit, null, 2), "utf8");

console.log(JSON.stringify({
  status: "BATCH_149_SAN_DIEGO_LIVE_VALIDATION_AUDIT_COMPLETE",
