...
const targets = {
  version: "black_dragon_tucson_live_validation_targets_v1",
  generated_at: new Date().toISOString(),
  city: "Tucson",
  state: "AZ",
  validation_targets: runtime.merged_entities.map((entity, index) => ({
    validation_id: `BD_TUC_HTTP_VALIDATE_${String(index + 1).padStart(3, "0")}`,
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/automation/live_validation/tucson/manifests/live_validation_targets.json"),
...
(async () => {
  ...
  const resultsPayload = {
    version: "black_dragon_tucson_live_http_validation_results_v1",
    generated_at: new Date().toISOString(),
  ...
  fs.writeFileSync(
    path.join(ROOT, "public/data/clients/black_dragon/automation/live_validation/tucson/results/live_http_validation_results.json"),
    JSON.stringify(resultsPayload, null, 2),
    "utf8"
  );

  const freshness = {
    version: "black_dragon_tucson_live_freshness_registry_v1",
    generated_at: new Date().toISOString(),
  ...
  fs.writeFileSync(
    path.join(ROOT, "public/data/clients/black_dragon/automation/live_validation/tucson/freshness/live_freshness_registry.json"),
    JSON.stringify(freshness, null, 2),
    "utf8"
  );

  const reviewItems = results.filter(r => r.fetch_status !== "VALID").map((row, index) => ({
    dead_route_id: `BD_TUC_ROUTE_REVIEW_${String(index + 1).padStart(5, "0")}`,
    validation_id: row.validation_id,
    organization_name: row.organization_name,
    route_type: row.route_type,
    url: row.url,
  ...
  fs.writeFileSync(
    path.join(ROOT, "public/data/clients/black_dragon/automation/live_validation/tucson/dead_routes/dead_route_review_queue.json"),
    JSON.stringify({ version: "black_dragon_tucson_dead_route_review_queue_v1", generated_at: new Date().toISOString(), review_item_count: reviewItems.length, dead_route_items: reviewItems }, null, 2),
    "utf8"
  );
  ...
  fs.writeFileSync(
    path.join(ROOT, "public/data/clients/black_dragon/automation/live_validation/tucson/contact_review/contact_route_review.json"),
    JSON.stringify({ version: "black_dragon_tucson_contact_route_review_v1", generated_at: new Date().toISOString(), review_count: contactReview.length, contact_ready_candidates: contactReview.filter(r => r.contact_ready_candidate).length, contact_review: contactReview }, null, 2),
    "utf8"
  );

  const audit = {
    version: "black_dragon_batch_166_tucson_live_validation_audit_v1",
    generated_at: new Date().toISOString(),
    batch: "166_TUCSON_LIVE_VALIDATION",
    counts: {
    ...
    next_phase: "BATCH_167_TUCSON_UI_AND_PRODUCTION_AUDIT",
    status: "PASS"
  };

  const out = path.join(ROOT, "public/data/clients/black_dragon/automation/live_validation/tucson/audit/batch_166_tucson_live_validation_audit.json");
  fs.writeFileSync(out, JSON.stringify(audit, null, 2), "utf8");

  console.log(JSON.stringify({
    status: "BATCH_166_TUCSON_LIVE_VALIDATION_AUDIT_COMPLETE",
