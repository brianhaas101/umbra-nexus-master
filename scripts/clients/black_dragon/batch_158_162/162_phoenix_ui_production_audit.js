...
const runtime = read("public/data/clients/black_dragon/city_runtime/phoenix/merged/phoenix_merged_city_entities.json");
const graph = read("public/data/clients/black_dragon/relationship_graph/phoenix/edges/phoenix_graph_edges.json");
const scores = read("public/data/clients/black_dragon/relationship_graph/phoenix/scores/phoenix_graph_influence_scores.json");
const paths = read("public/data/clients/black_dragon/relationship_graph/phoenix/paths/phoenix_propagation_paths.json");
const validation = read("public/data/clients/black_dragon/automation/live_validation/phoenix/results/live_http_validation_results.json");
const contactReview = read("public/data/clients/black_dragon/automation/live_validation/phoenix/contact_review/contact_route_review.json");
const deadRoutes = read("public/data/clients/black_dragon/automation/live_validation/phoenix/dead_routes/dead_route_review_queue.json");

const panel = {
  version: "black_dragon_phoenix_client_overview_panel_v1",
  generated_at: new Date().toISOString(),
  city: "Phoenix",
  state: "AZ",
  panel_id: "BD_PHX_CLIENT_OVERVIEW",
  panel_title: "Phoenix Operational Intelligence",
  client_visible: true,
  summary_cards: [
    { card_id: "PHX_RUNTIME_ENTITIES", label: "Runtime Entities", count: runtime.deduped_city_entities },
    { card_id: "PHX_CONTACT_CANDIDATES", label: "Manual Contact Candidates", count: contactReview.contact_ready_candidates },
    { card_id: "PHX_GRAPH_EDGES", label: "Graph Edges", count: graph.total_edges },
    { card_id: "PHX_PROPAGATION_PATHS", label: "Propagation Paths", count: paths.total_paths },
    { card_id: "PHX_ROUTE_REVIEW", label: "Route Reviews", count: deadRoutes.review_item_count },
    { card_id: "PHX_CROSS_STATE_DUPLICATES", label: "Cross-State Overlaps", count: runtime.duplicate_review_entities }
  ],
  hardlocks: {
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/phoenix/ui/panels/phoenix_client_overview_panel.json"),
...
const nodeCards = runtime.merged_entities.map(entity => {
  ...
  return {
    node_card_id: `BD_PHX_NODE_CARD_${String(entity.city_rank).padStart(5, "0")}`,
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/phoenix/ui/node_cards/phoenix_client_node_cards.json"),
  JSON.stringify({ version: "black_dragon_phoenix_client_node_cards_v1", generated_at: new Date().toISOString(), total_node_cards: nodeCards.length, node_cards: nodeCards }, null, 2),
  "utf8"
);

const feed = {
  version: "black_dragon_phoenix_propagation_feed_v1",
  generated_at: new Date().toISOString(),
  city: "Phoenix",
  state: "AZ",
  client_visible: true,
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/phoenix/ui/propagation_feed/phoenix_client_propagation_feed.json"),
  JSON.stringify(feed, null, 2),
  "utf8"
);

const uiAudit = {
  version: "black_dragon_batch_162_phoenix_ui_surface_audit_v1",
  generated_at: new Date().toISOString(),
  batch: "162_PHOENIX_UI_AND_PRODUCTION_AUDIT",
  counts: {
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/phoenix/ui/audit/batch_162_phoenix_ui_surface_audit.json"),
  JSON.stringify(uiAudit, null, 2),
  "utf8"
);

const prodAudit = {
  version: "black_dragon_phoenix_production_readiness_audit_v1",
  generated_at: new Date().toISOString(),
  city: "Phoenix",
  state: "AZ",
  certification_target: "ARIZONA_FIRST_OPERATIONAL_CITY",
  counts: {
  ...
  certification: "PHOENIX_OPERATIONAL_CITY_APPROVED",
  next_phase: "BATCH_163_SCOTTSDALE_RUNTIME_TEMPLATE_INITIALIZATION",
  status: "PASS"
};

const out = path.join(ROOT, "public/data/clients/black_dragon/production_readiness/phoenix/audit/phoenix_production_readiness_audit.json");
fs.writeFileSync(out, JSON.stringify(prodAudit, null, 2), "utf8");

console.log(JSON.stringify({
  status: "BATCH_162_PHOENIX_PRODUCTION_READINESS_AUDIT_COMPLETE",
