...
const dedupe = read("public/data/clients/black_dragon/candidate_queue/mesa/dedupe/mesa_regional_dedupe.json");
...
const merged = dedupe.deduped_candidates.map((candidate, index) => {
  ...
  return {
    city_runtime_entity_id: `BD_MSA_RUNTIME_ENTITY_${String(index + 1).padStart(5, "0")}`,
    ...
    source_layers: [candidate.source_category, "MESA_DISCOVERY_IMPORT"],
...
const runtime = {
  version: "black_dragon_mesa_runtime_merge_v1",
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/mesa/merged/mesa_merged_city_entities.json"),
  JSON.stringify(runtime, null, 2),
  "utf8"
);

const dossiers = runtime.merged_entities.map(entity => ({
  dossier_id: `BD_MSA_DOSSIER_${String(entity.city_rank).padStart(5, "0")}`,
  ...
  summary: `${entity.organization_name} is a Mesa motorcycle ecosystem candidate for Black Dragon propagation analysis.`,
  intelligence_fields: {
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/mesa/dossiers/mesa_runtime_dossiers.json"),
  JSON.stringify({ version: "black_dragon_mesa_runtime_dossiers_v1", generated_at: new Date().toISOString(), dossier_count: dossiers.length, dossiers }, null, 2),
  "utf8"
);
...
const mapNodes = runtime.merged_entities.map((entity, index) => ({
  map_node_id: `BD_MSA_MAP_NODE_${String(entity.city_rank).padStart(5, "0")}`,
  city_runtime_entity_id: entity.city_runtime_entity_id,
  organization_name: entity.organization_name,
  city_rank: entity.city_rank,
  priority_tier: entity.priority_tier,
  ...
  node_type: entity.regional_duplicate_detected ? "REGIONAL_OVERLAP_NODE" : "MESA_RUNTIME_NODE",
  runtime_visible: true,
  contact_ready: false,
  automated_outreach_allowed: false
}));
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/city_runtime/mesa/map_nodes/mesa_runtime_map_nodes.json"),
  JSON.stringify({ version: "black_dragon_mesa_runtime_map_nodes_v1", generated_at: new Date().toISOString(), map_node_count: mapNodes.length, map_nodes: mapNodes }, null, 2),
  "utf8"
);

const graphNodes = runtime.merged_entities.map(entity => ({
  graph_node_id: `BD_MSA_GRAPH_NODE_${String(entity.city_rank).padStart(5, "0")}`,
  organization_name: entity.organization_name,
  city_runtime_entity_id: entity.city_runtime_entity_id,
...
for (let i = 0; i < graphNodes.length; i++) {
  for (let j = i + 1; j < graphNodes.length; j++) {
    ...
    graphEdges.push({
      graph_edge_id: `BD_MSA_GRAPH_EDGE_${String(graphEdges.length + 1).padStart(6, "0")}`,
      ...
      relationship_type: a.regional_duplicate_detected || b.regional_duplicate_detected ? "ARIZONA_REGIONAL_OVERLAP" : "MESA_ECOSYSTEM_RELATIONSHIP",
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/relationship_graph/mesa/nodes/mesa_graph_nodes.json"),
  JSON.stringify({ version: "black_dragon_mesa_graph_nodes_v1", generated_at: new Date().toISOString(), total_nodes: graphNodes.length, nodes: graphNodes }, null, 2),
  "utf8"
);

fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/relationship_graph/mesa/edges/mesa_graph_edges.json"),
  JSON.stringify({ version: "black_dragon_mesa_graph_edges_v1", generated_at: new Date().toISOString(), total_edges: graphEdges.length, edges: graphEdges }, null, 2),
  "utf8"
);
...
const paths = scores.slice(0, 10).map((score, index) => ({
  propagation_path_id: `BD_MSA_PROP_PATH_${String(index + 1).padStart(5, "0")}`,
...
fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/relationship_graph/mesa/scores/mesa_graph_influence_scores.json"),
  JSON.stringify({ version: "black_dragon_mesa_graph_influence_scores_v1", generated_at: new Date().toISOString(), total_scored_nodes: scores.length, scores }, null, 2),
  "utf8"
);

fs.writeFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/relationship_graph/mesa/paths/mesa_propagation_paths.json"),
  JSON.stringify({ version: "black_dragon_mesa_propagation_paths_v1", generated_at: new Date().toISOString(), total_paths: paths.length, paths }, null, 2),
  "utf8"
);

const audit = {
  version: "black_dragon_batch_165_mesa_runtime_merge_graph_audit_v1",
  generated_at: new Date().toISOString(),
  batch: "165_MESA_RUNTIME_MERGE_AND_GRAPH",
  counts: {
  ...
  next_phase: "BATCH_166_MESA_LIVE_VALIDATION",
  status: "PASS"
};

const out = path.join(ROOT, "public/data/clients/black_dragon/city_runtime/mesa/audit/batch_165_mesa_runtime_merge_graph_audit.json");
fs.writeFileSync(out, JSON.stringify(audit, null, 2), "utf8");

console.log(JSON.stringify({
  status: "BATCH_165_MESA_RUNTIME_MERGE_GRAPH_AUDIT_COMPLETE",
