...
const validated = JSON.parse(fs.readFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/club_association_layer/validated/long_beach_club_association_validated.json"),
  "utf8"
));
...
function score(org) {
  return (
    ...
    org.propagation_score * 0.15
  );
}

const dossiers = validated.validated
  .map(org => ({
    dossier_id: `BD_CLUB_ASSOC_DOSSIER_${org.organization_registry_id}`,
    ...
    propagation_score: org.propagation_score,
    priority_tier:
      score(org) >= 8.75 ? "HOT" :
      score(org) >= 8.0 ? "WARM" :
      "REVIEW",
...
const out = path.join(
  ROOT,
  "public/data/clients/black_dragon/club_association_layer/dossiers/long_beach_club_association_ranked_dossiers.json"
);

fs.writeFileSync(out, JSON.stringify({
  version: "black_dragon_long_beach_club_association_ranked_dossiers_v1",
...
console.log(JSON.stringify({
  status: "LONG_BEACH_CLUB_ASSOCIATION_DOSSIERS_COMPLETE",
  total_dossiers: dossiers.length,
  output: out
}, null, 2));
