...
const validated = JSON.parse(fs.readFileSync(
  path.join(ROOT, "public/data/clients/black_dragon/online_distribution_layer/validated/long_beach_online_distribution_validated.json"),
  "utf8"
));
...
function score(org) {
  return (
    ...
    org.digital_distribution_score * 0.25
  );
}

const dossiers = validated.validated
  .map(org => ({
    dossier_id: `BD_ONLINE_DOSSIER_${org.organization_registry_id}`,
    ...
    digital_distribution_score: org.digital_distribution_score,
    priority_tier:
      score(org) >= 8.75 ? "HOT" :
      score(org) >= 8.0 ? "WARM" :
      "REVIEW",
...
const out = path.join(
  ROOT,
  "public/data/clients/black_dragon/online_distribution_layer/dossiers/long_beach_online_distribution_ranked_dossiers.json"
);

fs.writeFileSync(out, JSON.stringify({
  version: "black_dragon_long_beach_online_distribution_ranked_dossiers_v1",
...
console.log(JSON.stringify({
  status: "LONG_BEACH_ONLINE_DISTRIBUTION_DOSSIERS_COMPLETE",
  total_dossiers: dossiers.length,
  output: out
}, null, 2));
