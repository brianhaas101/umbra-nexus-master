...
const queue = counts.cities
  ...
  .map((city, index) => ({
    city_expansion_task_id:
      `BD_NORMALIZED_CITY_EXPANSION_TASK_${String(index + 1).padStart(5, "0")}`,
    ...
    existing_seed_examples:
      city.organization_names.slice(0, 10),
...
fs.writeFileSync(out, JSON.stringify({
  version: "black_dragon_normalized_city_expansion_gap_queue_v1",
...
console.log(JSON.stringify({
  status: "NORMALIZED_CITY_EXPANSION_GAP_QUEUE_COMPLETE",
  total_expansion_tasks: queue.length,
  output: out
}, null, 2));
